...
exports.processGeneration = processGeneration;
exports.pollGenerationStatus = pollGenerationStatus;
exports.finalizeGeneration = finalizeGeneration;
...
async function processGeneration(user, modelName, prompt, imageUrl, options = {}) {
    const modelConfig = MODEL_ENDPOINTS[modelName];
    if (!modelConfig)
        throw new Error('Model not found');
    const pricing = await (0, supabaseService_1.getModelPricing)(modelName);
    ...
    const keys = await (0, supabaseService_1.getApiKeysForUser)(user);
    if (!keys.length)
        throw new Error('Bot sedang sibuk (No API Keys).');
    let taskId = '';
    let usedKey = '';
    for (const key of keys) {
        try {
            console.log(`Using Key: ...${key.slice(-4)}`);
            const res = await axios_1.default.post(`${FREEPIK_API_BASE}${modelConfig.endpoint}`, payload, {
                headers: {
    ...
    if (!taskId)
        throw new Error('Semua API Key sibuk atau habis kuota.');
    if (user.type !== 'UNLIMITED' && user.type !== 'ADVANCE') {
        await (0, supabaseService_1.consumeCredits)(user.id, creditCost);
    }
    const genRecord = await (0, supabaseService_1.logGeneration)({
        user_id: user.id,
...
async function pollGenerationStatus(taskId, modelName, apiKey) {
    const statusEndpoint = MODEL_STATUS_ENDPOINTS[modelName] || '/image-to-video/kling-v2-1';
    try {
        const res = await axios_1.default.get(`${FREEPIK_API_BASE}${statusEndpoint}/${taskId}`, {
        ...
        if (e.response?.status === 404)
            return { status: 'processing' }; // Not found usually means queueing
        return { status: 'error', message: e.message };
    }
}
async function finalizeGeneration(generationId, videoUrl, userId, modelName) {
    try {
        ...
        const { uploadVideoToR2 } = await Promise.resolve().then(() => __importStar(require('./r2Service')));
        // Upload to R2
        let r2Url = null;
        try {
        ...
        // Update DB
        const { error } = await supabaseService_1.supabase.from('generations').update({
            status: 'completed',
            video_url: videoUrl,
        ...
        // Increment count
        await supabaseService_1.supabase.rpc('increment_video_count', { user_id: userId });
